...
export async function POST(req: Request) {
  try {
    ...
    const bookingNumber = body.description?.match(/ATR-[A-Z0-9]{6}/)?.[0];

    if (bookingNumber && (orderStatus === "APPROVED" || orderStatus === "PAID")) {
      await db
        .update(transferBookings)
        .set({
          paymentStatus: "paid",
          ...
          adminNotes: `Payriff Webhook: Order ${orderId}`,
          updatedAt: new Date(),
        })
        .where(eq(transferBookings.bookingNumber, bookingNumber));
    }
